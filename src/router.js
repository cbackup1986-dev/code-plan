 ...
 *   deep_reasoning → 复杂推理、架构设计、长规划    → DeepSeek-V3（推理最强）
 *   file_ops       → 文件读写、工具调用密集型      → Kimi-K2.5（工具调用稳定）
 *   long_context   → 超长上下文处理               → DeepSeek-V3（长窗口）
 */
...
// ─── LLM 意图识别（快速小模型）────────────────────────────────────────────
async function llmClassify(messages, tools, apiKey, endpoint) {
  const lastUserMsg = [...messages].reverse().find(m => m.role === 'user');
  const text = typeof lastUserMsg?.content === 'string'
    ? lastUserMsg.content
    : JSON.stringify(lastUserMsg?.content || '');
